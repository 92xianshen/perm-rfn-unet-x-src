...
from .permutohedralx_v3_computation import PermutohedralXComputation
...
class CRFComputation(tf.Module):
    ...
    def init_features(self, image: tf.Tensor, height: int, width: int, d_bifeats: int, d_spfeats: int, theta_alpha: float, theta_beta: float, theta_gamma: float) -> tf.Tensor:
    ...
    def init_lattice(self, features: tf.Tensor, N: int, d: int, canonical: tf.Tensor, E: tf.Tensor, scale_factor: tf.Tensor, diff_valid: tf.Tensor, d_mat: tf.Tensor, diagone: tf.Tensor) -> tf.Tensor:
        # - Initialize lattice
        lattice = PermutohedralXComputation()
        coords_1d_uniq, M, os, ws, ns = lattice.init(features, N, d, canonical, E, scale_factor, diff_valid, d_mat, diagone)

        return coords_1d_uniq, M, os, ws, ns

    @tf.function(input_signature=[
        tf.TensorSpec(shape=[None, None, None], dtype=tf.float32), # of unary, [H, W, D], float32
        tf.TensorSpec(shape=[], dtype=tf.int32), # of n_feats (a.k.a. N), [], int
        tf.TensorSpec(shape=[], dtype=tf.int32), # of num_classes, [], int
        tf.TensorSpec(shape=[], dtype=tf.int32), # of d_bifeats, [], int
        tf.TensorSpec(shape=[], dtype=tf.int32), # of d_spfeats, [], int
    ...
    def mean_field_approximation(self, unary: tf.Tensor, n_feats: int, num_classes: int, d_bifeats: int, d_spfeats: int, alpha_bilateral: float, alpha_spatial: float,  os_bilateral: tf.Tensor, ws_bilateral: tf.Tensor, blur_neighbors_bilateral: tf.Tensor, M_bilateral: int, os_spatial: tf.Tensor, ws_spatial: tf.Tensor, blur_neighbors_spatial: tf.Tensor, M_spatial: int, bilateral_compat: float, spatial_compat: float, compatibility: float, num_iterations: int) -> tf.Tensor:
        # - Create lattice
        lattice = PermutohedralXComputation()

        # - Compute symmetric weights
        all_ones = tf.ones([n_feats, 1], dtype=tf.float32) # [N, 1]
        bilateral_norm_vals = lattice.compute(all_ones, n_feats, d_bifeats, alpha_bilateral, os_bilateral, ws_bilateral, blur_neighbors_bilateral, M_bilateral)
        bilateral_norm_vals = 1. / (bilateral_norm_vals ** .5 + 1e-20)
        spatial_norm_vals = lattice.compute(all_ones, n_feats, d_spfeats, alpha_spatial, os_spatial, ws_spatial, blur_neighbors_spatial, M_spatial)
        ...
        for i in range(num_iterations):
            ...
            bilateral_out = lattice.compute(Q * bilateral_norm_vals, n_feats, d_bifeats, alpha_bilateral, os_bilateral, ws_bilateral, blur_neighbors_bilateral, M_bilateral) # [N, C]
            bilateral_out *= bilateral_norm_vals # [N, C]

            # - Symmetric normalization and spatial message passing
            spatial_out = lattice.compute(Q * spatial_norm_vals, n_feats, d_spfeats, alpha_spatial, os_spatial, ws_spatial, blur_neighbors_spatial, M_spatial) # [N, C]
