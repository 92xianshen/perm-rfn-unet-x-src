...
- 2023.07.27: Experiment with `tf.lookup.experimental.MutableHashTable`, denoted v5.
"""
...
class PermutohedralX(tf.Module):
    def __init__(self, d: int) -> None:
        ...
        self.alpha = tf.constant(
            1.0 / (1.0 + tf.pow(2.0, -tf.cast(d, dtype=tf.float32))), 
            dtype=tf.float32, 
        )

        # - Helper constant values in matrix forms.
        ...
        # - 2023.07.27: Create a dynamic hash table
        self.table = tf.lookup.experimental.MutableHashTable(
            key_dtype=tf.int32, 
    ...
    def init(self, features: tf.Tensor) -> tf.Tensor:
        """
        Computation of the initialization of the permutohedral lattice. Pure TF operations. Get tf.Tensor.
        """
        # - Get `N` here.
        ...
        # - 2023.07.27: Use inner hash table.
        M = tf.shape(coords_1d_uniq)[0]
        self.table.insert(coords_1d_uniq, tf.range(M, dtype=tf.int32))
        ...
        blur_neighbors = tf.stack([self.table.lookup(n1s), self.table.lookup(n2s)], axis=0) # [2, M, d + 1]
        # - 2023.07.27: Reset the hash table.
        self.table.remove(coords_1d_uniq)

        # - Shift all values by 1 such that -1 -> 0 (used for blurring)
        os = tf.reshape(offsets, shape=[-1, ]) + 1 # [N x (d + 1), ]
        ws = tf.reshape(barycentric[..., :self.d + 1], shape=[-1, ]) # [N x (d + 1), ]
        blur_neighbors = blur_neighbors + 1 # [2, M, d + 1]

        return os, ws, blur_neighbors, M

    @tf.function(input_signature=[
        tf.TensorSpec(shape=[None, None], dtype=tf.float32), # of inp, should be flattened, [N, value_size], float32
    ...
    def compute(self, inp: tf.Tensor, os: tf.Tensor, ws: tf.Tensor, blur_neighbors: tf.Tensor, M: int, reverse: bool=False) -> tf.Tensor:
        """
        Compute sequentially.

        Args:
            inp: input to be filtered, [size (a.k.a. N), value_size], float32, channel-last.
            ...
            reverse: indicating the blur order, false by default.
